    ...
    3) for each item, matches a primary filter? If so, for each item matches the paired secondary filter? if so do collision?
      ...
      - so again, im at this filter, collide, act loop. But so I can get started generalise later, after working code!
        ...
        for (loop(world))
        {
            ...
            if (withconstants) colliders.push_back(loop);
        }
...
struct GameCode
{
    using TestSingle      = function<bool(const Entity&)>;
    using TestPair        = function<bool(const Entity&, const Entity&)>;

    using ActionThink     = function<Entity(Entity, const EntityConstants&)>;
    using ActionCreate    = function<pair<Entity, Entity>(Entity)>;
    using ActionMap       = function<Entity(Entity)>;
    using ActionMapPair   = function<pair<Entity, Entity>(Entity, Entity)>;
...
// /////////////////////
// NOTE: This is meant to be iterated. Just write it so it works, then generalise it!
// /////////////////////
vector<Entity> OneGameTick(const vector<Entity>& oldWorld, GameCode theGame)
{
    ...
    //          Also get a list of thinkers and nones.
